        ...
        {days.map((day) => (
          ...
          >
            <div
              ...
              style={{ height: `${Math.max(4, (day.total / peak) * 100)}%` }}
            >
              {/* Reached-a-person portion sits on top of the total. */}
              <div
                className="w-full bg-accent/35"
                style={{
                  height: `${day.total ? ((day.total - day.reached) / day.total) * 100 : 0}%`,
                }}
              />
              <div
                className="w-full bg-good"
                style={{
                  height: `${day.total ? (day.reached / day.total) * 100 : 0}%`,
                }}
              />
            </div>

            <span className="text-[0.6rem] text-ink-faint">{day.label}</span>

            <div className="pointer-events-none absolute -top-8 z-10 rounded-md border border-border bg-surface px-2 py-1 text-xs whitespace-nowrap opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
